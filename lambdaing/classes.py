...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        best_card = None
        if player.hand:
            p=[card.power(opponent_card) for card in player.hand if not isinstance(card, TutorCard)]
            p1=[card.power(opponent_card) for card in player.hand]
            best_card=player.hand[p1.index(max(p))]
