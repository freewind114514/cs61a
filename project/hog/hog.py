...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2    
    return 3*abs(player_score%10-opponent_score//10%10) or 1
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 
    for a in range(0,goal):
        for b in range(0,goal):
            for c in range(0,goal):
                for d in range(0,goal):
                    if strategy(a,b)!=strategy(c,d):
                        return False
    return True  
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    while i<10:
        i+=1
        current_averge=average_score(i,dice)
        if max_average_score<current_averge:
            max_average_score=current_averge
            if k<i:
                k=i
    return k
    # END PROBLEM 9
...
def final_strategy(score, opponent_score,threshold=10, num_rolls=5):
    """Write a brief description of your final strategy.

    *** YOUR DESCRIPTION HERE ***
    """
    sus=sus_points(score+boar_brawl(score, opponent_score))-score
