...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def helper(place,i):
            if place.is_hive:
                return None
            if self.min_range<=i<=self.max_range:
                if place.bees!=[]:
                    return random_bee(place.bees)

            return helper(place.entrance,i+1)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            for bee in copy:
                bee.reduce_health(self.health+self.damage)
        else:
            for bee in copy:
    ...
    def action(self, gamestate):
        ...
        def helper(place):
            ...
            if place.ant:
                if place.ant.double:
                    place.ant.damage *=2
                    place.ant.double=False
                if place.ant.is_container:
                    if place.ant.ant_contained:
                        if place.ant.ant_contained.double:
                            place.ant.ant_contained.damage *=2
                            place.ant.ant_contained.double=False

            return helper(place.exit)
        helper(self.place.exit)
        # END Problem 12
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and target.scare_endurance is False:
            target.scare(2)
            target.scare_endurance=True
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        def helper(place,distance):
            ...
            if place.ant and place.ant is not self:
                result[place.ant]=distance
                if place.ant.is_container:
                    if place.ant.ant_contained and place.ant.ant_contained is not self:
                        result[place.ant.ant_contained]=distance
            if place.bees:
                for bee in place.bees:
                    result[bee]=distance
            return helper(place.entrance,distance+1)
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(self.damage-0.25*distance-self.insects_shot*0.0625,0)
        # END Problem Optional 2
