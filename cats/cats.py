...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    p = [s for s in paragraphs if select(s)]
    if len(p) - 1 < k:
        return ""
    else:
        return p[k]
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def f(s):
        lst = split(lower(remove_punctuation(s)))
        for i in lst:
            for k in topic:
                if i == k:
                    return True
...
def accuracy(typed, reference):
    ...
    for i in range(len(typed_words)):
        for k in range(len(reference_words)):
            if typed_words[i] == reference_words[k] and i == k:
                a += 1
    if len(reference_words) == len(typed_words) == 0:
        return 100.0
    elif len(reference_words) == 0 or len(typed_words) == 0:
        return 0.0
    else:
        return 100 * a / len(typed_words)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    diff = [diff_function(w1, typed_word, limit) for w1 in word_list]
...
def sphinx_swaps(start, goal, limit):
    ...
    def helper(i, j, cnt):
        if cnt > limit:
            return limit + 1
        if i + 1 > len(start) or j + 1 > len(goal):
            return cnt + abs(len(start) - len(goal))
        if start[i] == goal[j]:
            return helper(i + 1, j + 1, cnt)
        if start[i] != goal[j]:
            return helper(i + 1, j + 1, cnt + 1)
...
def minimum_mewtations(start, goal, limit):
    ...
    def helper(i, j, count):
        ...
        if i >= len(start) or j >= len(goal):
            return count + abs(len(start[i:]) - len(goal[j:]))
        add = helper(i, j + 1, count + 1)
        remove = helper(i + 1, j, count + 1)
        substitute = helper(i + 1, j + 1, count + 1)
        if start[i] == goal[j]:
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(prompt)):
        if i == len(sofar) or sofar[i] != prompt[i]:
            upload({"id": user_id, "progress": len(sofar[:i]) / len(prompt)})
            return len(sofar[:i]) / len(prompt)
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for j in range(len(times_per_player)):
        times.append(
            [
                times_per_player[j][i + 1] - times_per_player[j][i]
...
def fastest_words(match):
    ...
    for j in word_indices:
        speed.append([time(match, i, j) for i in player_indices])
    for j in word_indices:
        for i in player_indices:
            if speed[j][i] == min(speed[j]):
                fast += [i]
                speed[j] += [-1]

    for i in player_indices:
        result.append([word_at(match, j) for j in word_indices if fast[j] == i])
    return result
    # END PROBLEM 10
