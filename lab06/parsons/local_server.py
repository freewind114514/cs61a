...
def get_problems():
    ...
    req_names = [
        f"{pname} {CHECK_MARK if probs_correct[pname] else RED_X}"
        for pname in cache[REQUIRED_PROBLEMS]
    ]
    req_paths = [f"/code_skeleton/{pname}" for pname in cache[REQUIRED_PROBLEMS]]
    opt_names = [
        f"{pname} {CHECK_MARK if probs_correct[pname] else RED_X}"
        for pname in cache[OPTIONAL_PROBLEMS]
    ]
...
def write_parsons_prob_locally(path, code, parsons_repr_code, write_repr_code):
    ...
    assert (
        start_line >= 0
    ), f"Problem not found in file {path}. This can be due to missing doctests."

    code_lines = code.split("\n")
    assert (
        "def" in code_lines[0] or "class" in code_lines[0]
    ), "First code block must be the `def` statement or `class` declaration"
...
def grade_and_backup(problem_name):
    ...
    feedback["failed"] = (
        assign.specified_tests[0].console.cases_total - feedback["passed"]
    )
...
def get_useful_syntax_error_logs(logs, problem_name):
    ...
    logs[file_index] = re.sub(
        r"line ([0-9]+)",
        f"line {original_line_num - docstring_lines}",
        logs[file_index],
    )
...
def setup():
    ...
    try:
        ...
        print(
            f"Error while loading assignment: {str(e)}. This is likely due to a syntax error in the mentioned file."
        )
        exit(1)
